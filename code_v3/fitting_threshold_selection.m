...
% figure
% x = min(errs)+((1/number)*(max(feature_errs) - min(errs))):(1/number)*(max(feature_errs) - min(errs)):max(feature_errs);
% plot(x,y,'r','LineWidth',3.0); hold on; plot(x,feature_y,'c','LineWidth',3.0); hold on
